...
def delta_cell(delta_units, needs_trade):
    """Render a coloured delta units cell, with HOLD if within turnover band."""
...
if portfolio_val > 0 and fx_rate:
    ...
    for row in allocation:
        ...
        if row["investable"]:
            ...
            if ibkr_positions and ticker in ibkr_positions:
                current_units = ibkr_positions[ticker]["quantity"]
                delta = target_units - current_units
                current_units_display = f"{current_units:.3f}"
            else:
                current_units = None
                delta = target_units  # First buy — full target
            ...
            d_display = delta_cell(delta, trade_required)
        else:
            # Cash slot
            target_display = "<span class='neu'>USD cash</span>"
            current_units_display = "<span class='neu'>—</span>"
    ...
    st.markdown(
        f"<table>"
        f"<tr><th>Ticker</th><th>Status</th><th>Target Wt</th><th>Actual Wt</th><th>Drift</th><th>SGD Target</th><th>USD Target</th><th>Target Units</th><th>Current Units</th><th>Δ Trade</th></tr>"
        f"{rows3}"
        f"</table>",
        unsafe_allow_html=True
    )
