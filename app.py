...
def fetch_ibkr_positions():
    ...
    try:
        r1 = requests.get(base_url, params={"t": token, "q": query_id, "v": "3"}, timeout=15)
        r1.raise_for_status()
    ...
    for attempt in range(4):
        try:
            r2 = requests.get(download_url, params={"t": token, "q": reference_code, "v": "3"}, timeout=15)
            r2.raise_for_status()
            if "<FlexQueryResponse" in r2.text or "<OpenPosition" in r2.text:
                break
...
if ibkr_error:
    st.markdown(f'<div class="ibkr-warn">⚠ IBKR positions unavailable: {ibkr_error}. Delta column will show estimated units from portfolio value only.</div>', unsafe_allow_html=True)

# Show IBKR positions status
if ibkr_positions:
    pos_tickers = ", ".join(ibkr_positions.keys()) if ibkr_positions else "none"
